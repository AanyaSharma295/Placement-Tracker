...
export default function MCQCard({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `/api/subject-questions/${question.id}/attempt`,
        {
          method: "POST",
  ...
  function getOptionStyle(index: number) {
    ...
    return "border-gray-700 bg-gray-700/30 text-gray-500";
  }
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <span
          className={`text-xs px-2 py-0.5 rounded-full font-medium ${
            DIFFICULTY_STYLES[
              question.difficulty as keyof typeof DIFFICULTY_STYLES
        ...
        <span
          className={`text-xs px-2 py-0.5 rounded-full font-medium ${
            FREQUENCY_STYLES[
              question.frequency as keyof typeof FREQUENCY_STYLES
            ] ?? "bg-gray-700 text-gray-400"
          }`}
        >
          ...
          >
            ...
            <div
              className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all
                ${
                  revealed && index === result?.correctOption
                    ? "border-green-400"
                    : revealed && index === selected && !result?.isCorrect
                    ? "border-red-400"
                    : selected === index
                    ? "border-indigo-400"
                    : "border-gray-500"
